...
class Caregiver(db.Model):
    ...
    patients = db.relationship('Caregiver', backref='caregiver', lazy=True)
...
class Patient(db.Model):
    id= db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(120), nullable=False)
    facility = db.Column(db.String(120), index=True, unique=True, nullable=False)
    unit = db.Column(db.String(12), unique=True, nullable=False)
    medications = db.relationshipt('medications', backref='patient')
